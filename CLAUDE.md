...
5. **Drie pagina's, één app.js.** `web/index.html` (netplay, de gewone versie), `web/stream/` (gearchiveerde streamversie) en `web/2/` (doorverwijzing). De archiefpagina hergebruikt `web/app.js`, `web/style.css` en de wasm één map hoger via `window.VPH_BASE`/`VPH_API`. Nooit kopiëren "omdat het even sneller is" — twee kopieën lopen gegarandeerd uit elkaar, en juist bij netplay moeten beide kanten dezelfde core draaien. `build.sh` zet de cache-buster in alle drie.
6. **Determinisme is heilig in `web/netplay.js`.** Alles wat de emulatiestaat raakt (invoer, consoletoetsen, reset, regio, cartridge-wissel) moet via de lockstep en op een frame gepland worden — nooit rechtstreeks naar de core. Wijzig je daar iets, draai dan `make netcheck` in de Core én `tests/run.sh` hier. En bouw geen herstelmechanisme op een vermoeden: de stall-resync van 27-07 ging juist de verkeerde kant op (zie BUGLIST).
