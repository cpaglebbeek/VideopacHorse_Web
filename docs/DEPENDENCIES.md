...
| `build.sh` | `../VideopacHorse_Core`, `../Meta_VideopacHorse`, `version.json` | zet cache-busters in alle drie de pagina's. Voeg je een vierde pagina toe en vergeet je die hier, dan draait die met een oude wasm — precies wat bij netplay niet mag. Haalt daarnaast twee dingen uit zusterrepo's op: de wasm uit de Core en **AP-01** uit de sub-master (zie hieronder) |
| `web/architectuur/sessiereis.html` | `../Meta_VideopacHorse/architectuur/VideopacHorse_AP01_sessiereis.html` | build-artefact, net als de wasm — **nooit hier bewerken**. AP-01 gaat over het hele ecosysteem en hoort daarom in de sub-master; deze repo publiceert hem alleen. `build.sh` vult de `@VERSION@`-marker. Ontbreekt de sub-master, dan waarschuwt de build en blijft de oude versie staan in plaats van te falen |
